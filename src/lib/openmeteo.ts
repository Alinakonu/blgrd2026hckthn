...
export async function fetchProjectedClimate(
  ...
  const res = await fetchWithTimeout(
    `https://climate-api.open-meteo.com/v1/climate?${params}`,
    15_000,
  );
...
export function scoreStress(baseline: ClimateNormals, projected: ClimateNormals) {
  const heatDeltaC = projected.meanTempC - baseline.meanTempC;
  const precipChangePct =
    ((projected.precipMmYear - baseline.precipMmYear) /
      Math.max(baseline.precipMmYear, 1)) *
    100;
  ...
  const heatScore = Math.min(
    ...
    Math.max(
      0,
      heatDeltaC * 28 + (projected.hotDaysApprox - baseline.hotDaysApprox) * 1.2,
    ),
  );
  ...
  const extremePrecipScore = Math.min(
    ...
    Math.max(
      0,
      heatDeltaC * 12 + (precipChangePct > 0 ? precipChangePct * 1.5 : 18),
    ),
  );
